...
| 2026-05-01 | MAS Sprint 1 · Dialectical Debate Framework | Replaced dual-voter ensemble in orchestrator.process_signal() with 3-call structured debate: DebateResult (bull+bear, 1 call) → AnalystSynthesis (1 call) → TraderDecision (1 call). All outputs via Pydantic with_structured_output (zero regex). Provenance chain: bull_evidence/bear_evidence → key_evidence → cited_evidence. Deterministic kill-switch short-circuit before any LLM spend. Added "debate" tier (temp=0.0, 200t) to factory.py. Removed secondary_voter from __init__. 64/64 Playwright tests passing. | MEMO/ChromaDB deferred to Sprint 2. Future sprints: Quantamental RAG (SEC filings), Vision-AI crypto, Grid trading, Options Greek alpha, FINSABER backtesting. |
